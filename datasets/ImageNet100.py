...
imagenet_classnames = [
    "dummy", "tench", "goldfish", "great white shark", "tiger shark", "hammerhead shark", "electric ray",
    "stingray", "rooster", "hen", "ostrich", "brambling", "goldfinch", "house finch", "junco",
    "indigo bunting", "American robin", "bulbul", "jay", "magpie", "chickadee", "American dipper",
    "kite", "bald eagle", "vulture", "great grey owl", "fire salamander",
    ...
    "ram", "bighorn sheep", "Alpine ibex", "hartebeest", "impala",
    ...
    "bathtub", "station wagon", "lighthouse", "beaker", "military hat",
    ...
    "lemon", "fig", "pineapple", "banana", "jackfruit", "cherimoya", "pomegranate",
...
class ImageNet100(Dataset):
    def __init__(self, root, split='train', preprocess=None):
        ...
        self._preprocessed_dir = Path(root) / 'preprocessed_data' / split
        self._typographic_dir = Path(root) / 'typographic_attack_data_3fonts' / split
    ...
    def __getitem__(self, idx):
        (image, typographic_image), label = self._samples[idx], self._labels[idx]
        image, typographic_image = Image.open(image), Image.open(typographic_image)
    ...
    def _make_typographic_attack_dataset(self) -> None:
        ...
        for i, file in enumerate(self._files):
            print(file)
            # make_image_text(file.relative_to(self._data_dir), imagenet_classnames, self._data_dir, self._typographic_dir, 0)
            make_image_text(file.relative_to(self._data_dir), self.classes, self._data_dir, self._typographic_dir, self._labels[i])